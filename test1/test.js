if (typeof MUSIC == "undefined" || !MUSIC) var MUSIC = {
    version: "1.0",
    _QMFL: true,
    _debugMode: false
};
...
MUSIC.returnFn = function(v) {
    ...
    if (window.ActiveXObject || window.msIsStaticHTML) {
        ua.ie = 6; (window.XMLHttpRequest || agent.indexOf("MSIE 7.0") > -1) && (ua.ie = 7); (window.XDomainRequest || agent.indexOf("Trident/4.0") > -1) && (ua.ie = 8);
        ...
        optmz = function(st) {
            return function(fns, tm) {
                var aargs;
                if (typeof fns == "string") return st(fns, tm);
                else {
        ...
        if ((m = /Chrome.(\d+\.\d+)/i.exec(agent)) || window.chrome) ua.chrome = m ? parseFloat(m[1], 10) : "2.0";
        else if ((m = /Version.(\d+\.\d+)/i.exec(agent)) || window.safariHandler) ua.safari = m ? parseFloat(m[1], 10) : "3.3";
...
if (MUSIC.userAgent.ie && (MUSIC._setTimeout && MUSIC._setInterval)) eval((MUSIC.userAgent.ie < 9 ? "var document = MUSIC._doc;": "") + "var setTimeout = MUSIC._setTimeout, setInterval = MUSIC._setInterval");
var ua = MUSIC.userAgent;
MUSIC.object = {
    ...
    extend: function() {
        ...
        for (; i < len; i++) if ((opts = arguments[i]) != null) for (var name in opts) {
            ...
            if (deep && (copy && (typeof copy === "object" && !copy.nodeType))) {
                if (src) clone = src;
                else if (MUSIC.lang.isArray(copy)) clone = [];
                else if (MUSIC.object.getType(copy) === "object") clone = {};
                else clone = copy;
    ...
    ease: function(src, tar, rule) {
        if (tar) {
            if (typeof rule != "function") rule = MUSIC.object._eachFn;
            MUSIC.object.each(src,
            function(v, k) {
...
MUSIC.console = function(expr) {
    if (window.console) if (console.assert) console.assert.apply(null, arguments);
    else expr || console.log.apply(null, slice.call(arguments, 1))
};
...
MUSIC.string = {
    ...
    listReplace: function(s, l) {
        if (MUSIC.lang.isHashMap(l)) {
            for (var i in l) s = MUSIC.string.commonReplace(s, l[i], i);
            return s
        } else return s + ""
    },
    trim: function(str) {
    ...
    URIencode: function(str) {
        var cc, ccc;
        return (str + "").replace(MUSIC.string.RegExps.URIencode,
        function(a) {
            if (a == " ") return "+";
            else if (a == "\r") return "";
            cc = a.charCodeAt(0);
    ...
    replaceUrl: function(str) {
        return str.replace(/%/g, "%25").replace(/=/g, "%3D").replace(/&/g, "%26").replace(/\?/g, "%3F").replace(/#/g, "%23")
    },
    decodeURLSymbol: function(str) {
        return str.replace(/%3d/g, "=").replace(/%3f/g, "?").replace(/%26/g, "&").replace(/%3c/g, "<").replace(/%3e/g, ">").replace(/%22/g, '"').replace(/%27/g, "'").replace(/\+/g, " ").replace(/%2f/ig, "/").replace(/%3a/g, ":")
    },
    GbkUrlDecode: function(str, callback) {
        ...
        if ( !! navigator && ("systemLanguage" in navigator && navigator.systemLanguage != "zh-cn")) jsLoader.load(MUSIC.config.toolPath + "js/gbk_decode.js");
        else {
            var opts = {
                type: "text/vbscript"
            };
    ...
    restHTML: function(str) {
        if (!MUSIC.string.restHTML.__utilDiv) MUSIC.string.restHTML.__utilDiv = document.createElement("div");
        var t = MUSIC.string.restHTML.__utilDiv;
        t.innerHTML = str + "";
        if (typeof t.innerText != "undefined") return t.innerText;
        else if (typeof t.textContent != "undefined") return t.textContent;
        else if (typeof t.text != "undefined") return t.text;
        else return ""
    },
    ...
    write: function(strFormat, someArgs) {
        if (arguments.length < 1 || !MUSIC.lang.isString(strFormat)) return "";
        var rArr = MUSIC.lang.arg2arr(arguments),
        result = rArr.shift(),
        tmp;
        return result.replace(MUSIC.string.RegExps.write,
        function(a, b, c) {
            b = parseInt(b, 10);
            if (b < 0 || typeof rArr[b] == "undefined") return "(n/a)";
            else if (!c) return rArr[b];
            else switch (c) {
            case "x":
    ...
    getRealLen: function(s, isUTF8) {
        ...
        if (!isUTF8) return s.replace(MUSIC.string.RegExps.getRealLen.r0, "**").length;
        else {
    ...
    format: function(str) {
        ...
        return str.replace(MUSIC.string.RegExps.format,
        function(m, n) {
    ...
    checkKoreaChar: function(str) {
        for (i = 0; i < str.length; i++) if (str.charCodeAt(i) > 12592 && str.charCodeAt(i) < 12687 || str.charCodeAt(i) >= 44032 && str.charCodeAt(i) <= 55203) return true;
        return false
    },
    escapeKoreaChar: function(str) {
        var dest = [];
        for (var i = 0; i < str.length; i++) if (str.charCodeAt(i) > 12592 && str.charCodeAt(i) < 12687 || str.charCodeAt(i) >= 44032 && str.charCodeAt(i) <= 55203) dest.push("&#" + str.charCodeAt(i) + ";");
        else dest.push(str.charAt(i));
        return dest.join("")
    },
...
MUSIC.event = {
    ...
    addEvent: function(obj, eventType, fn, argArray) {
        ...
        l;
        if (!obj) return res;
        if (!obj.eventsListUID) obj.eventsListUID = "e" + ++MUSIC.event._objSeqUID;
        if (! (l = MUSIC.event._eventListDictionary[obj.eventsListUID])) l = MUSIC.event._eventListDictionary[obj.eventsListUID] = {};
        if (!fn.__elUID) fn.__elUID = "e" + ++MUSIC.event._fnSeqUID + obj.eventsListUID;
        ...
        function withinElement(handler) {
            return function(e) {
                ...
                while (_p && _p != this) try {
                    _p = _p.parentNode
                } catch(e) {
                    _p = this
                }
                if (_p != this) handler.call(this, e)
            }
        }
        if (obj.addEventListener) {
            if (eventType === "mouseenter") obj.addEventListener("mouseover", withinElement(cfn), false);
            else if (eventType === "mouseleave") obj.addEventListener("mouseout", withinElement(cfn), false);
            else obj.addEventListener(eventType, cfn, false);
            res = true
    ...
    removeEvent: function(obj, eventType, fn) {
        ...
        r;
        if (!obj) return res;
        if (!fn) return MUSIC.event.purgeEvent(obj, eventType);
    ...
    purgeEvent: function(obj, type) {
        var l;
        if (obj.eventsListUID && ((l = MUSIC.event._eventListDictionary[obj.eventsListUID]) && l[type])) for (var k in l[type]) if (obj.removeEventListener) obj.removeEventListener(type, l[type][k], false);
    ...
    getEvent: function(evt) {
        ...
        if (!evt && window.Event) {
            ...
            cnt = 0;
            while (c) {
                if ((evt = c.arguments[0]) && typeof evt.srcElement != "undefined") break;
                else if (cnt > 9) break;
                c = c.caller; ++cnt
            }
        }
    ...
    getButton: function(evt) {
        ...
        if (!e) return - 1;
        if (MUSIC.userAgent.ie) return e.button - Math.ceil(e.button / 2);
        else return e.button
    },
    ...
    getCurrentTarget: function(evt) {
        var e = MUSIC.event.getEvent(evt);
        if (e) return e.currentTarget || document.activeElement;
        else return null
    },
    cancelBubble: function(evt) {
        ...
        else if (!evt.cancelBubble) evt.cancelBubble = true
    },
    ...
    replaceAllEvent: function(container) {
        ...
        function replaceElmEvent(elm, index) {
            if (!elm) return;
            MUSIC.object.each(["dblclick", "mouseover", "mouseout", "mousemove", "error", "click"],
            function(event_type) {
        ...
        for (var i = 0,
        len = _arr.length; i < len; i++) replaceElmEvent(_arr[i], i)
    },
    onDomReady: function(fn) {
        ...
        if (document.addEventListener) if (MUSIC.userAgent.safari < 4) var interval = setInterval(function() {
        ...
        50);
            ...
            if (/complete/.test(document.readyState)) MUSIC.event.onDomReady._fn();
            document.addEventListener("DOMContentLoaded", MUSIC.event.onDomReady._fn, true)
        } else {
            var src = window.location.protocol == "https:" ? "//:": "javascript:void(0)";
            document.write('<script onreadystatechange="if(this.readyState==\'complete\'){this.parentNode.removeChild(this);MUSIC.event.onDomReady._fn();}" defer="defer" src="' + src + '">\x3c/script>')
        }
    }
};
...
MUSIC.lang = {
    ...
    objectClone: function(obj, preventName) {
        if (typeof obj == "object") {
            ...
            for (var i in obj) if (i != preventName) res[i] = MUSIC.lang.objectClone(obj[i], preventName);
            return res
        } else if (typeof obj == "function") return (new obj).constructor;
        return obj
    },
    propertieCopy: function(s, b, propertiSet) {
        if (typeof propertiSet == "undefined") for (var p in b) s[p] = b[p];
        else for (var p in propertiSet) s[p] = b[p];
        return s
    },
    chain: function(u, v) {
        var calls = [];
        for (var ii = 0,
        len = arguments.length; ii < len; ii++) calls.push(arguments[ii]);
        return function() {
            for (var ii = 0,
            len = calls.length; ii < len; ii++) if (calls[ii] && calls[ii].apply(null, arguments) === false) return false;
            return true
        }
    },
    obj2str: function(obj) {
        ...
        if (typeof obj == "object") {
            ...
            t = [];
            for (var i in obj) t.push((sw ? "": '"' + MUSIC.string.escString(i) + '":') + MUSIC.lang.obj2str(obj[i]));
            t = t.join();
            return sw ? "[" + t + "]": "{" + t + "}"
        } else if (typeof obj == "undefined") return "undefined";
        else if (typeof obj == "number" || typeof obj == "function") return obj.toString();
    ...
    toArray: function(iterable) {
        ...
        if (iterable.toArray) return iterable.toArray();
            ...
            var results = [];
            for (var i = 0,
            length = iterable.length; i < length; i++) results.push(iterable[i]);
            return results
        }
    }
};
...
MUSIC.dom = {
    ...
    getByName: function(name, tagName) {
        ...
        for (var i = 0; i < e.length; ++i) if ( !! e[i].getAttribute("name") && e[i].getAttribute("name").toLowerCase() == name.toLowerCase()) arr.push(e[i]);
        return arr
    },
    get: function(e) {
    ...
    removeElement: function(elem) {
        if (elem = MUSIC.dom.get(elem)) {
            if (MUSIC.userAgent.ie == 9 && elem.tagName == "SCRIPT") elem.src = "";
    ...
    searchElementByClassName: function(elem, className) {
        ...
        return MUSIC.dom.searchChain(elem, "parentNode",
        function(el) {
    ...
    getElementsByClassName: function(className, tag, root) {
        ...
        for (var i = 0,
        len = elements.length; i < len; ++i) if (re.test(elements[i].className)) nodes[nodes.length] = elements[i];
        return nodes
    },
    ...
    getAncestorBy: function(elem, method) {
        elem = MUSIC.dom.get(elem);
        return MUSIC.dom.searchChain(elem.parentNode, "parentNode",
        function(el) {
    ...
    getFirstChild: function(elem) {
        elem = MUSIC.dom.get(elem);
        return elem.firstElementChild || MUSIC.dom.searchChain(elem && elem.firstChild, "nextSibling",
        function(el) {
    ...
    getLastChild: function(elem) {
        elem = MUSIC.dom.get(elem);
        return elem.lastElementChild || MUSIC.dom.searchChain(elem && elem.lastChild, "previousSibling",
        function(el) {
            return el.nodeType == 1
        })
    },
    getNextSibling: function(elem) {
        elem = MUSIC.dom.get(elem);
        return elem.nextElementSibling || MUSIC.dom.searchChain(elem && elem.nextSibling, "nextSibling",
        function(el) {
            return el.nodeType == 1
        })
    },
    getPreviousSibling: function(elem) {
        elem = MUSIC.dom.get(elem);
        return elem.previousElementSibling || MUSIC.dom.searchChain(elem && elem.previousSibling, "previousSibling",
        function(el) {
            return el.nodeType == 1
        })
    },
    swapNode: function(node1, node2) {
        if (node1.swapNode) node1.swapNode(node2);
        else {
            var prt = node2.parentNode,
            next = node2.nextSibling;
            if (next == node1) prt.insertBefore(node1, node2);
            else if (node2 == node1.nextSibling) prt.insertBefore(node2, node1);
            else {
    ...
    createElementIn: function(tagName, elem, insertFirst, attrs) {
        ...
        k;
        if (attrs) for (k in attrs) if (k == "class") _e.className = attrs[k];
        else if (k == "style") _e.style.cssText = attrs[k];
        else _e[k] = attrs[k];
    ...
    setStyle: function(el, properties, value) {
        if (! (el = MUSIC.dom.get(el)) || el.nodeType != 1) return false;
        var tmp, bRtn = true,
        ...
        for (var prop in properties) {
            ...
            if (prop == "float") prop = w3cMode ? "cssFloat": "styleFloat";
    ...
    getPosition: function(elem) {
        ...
        if (box = MUSIC.dom.getRect(elem)) {
            if (s = MUSIC.dom.getScrollLeft(doc = elem.ownerDocument)) box.left += s,
            box.right += s;
            if (s = MUSIC.dom.getScrollTop(doc)) box.top += s,
            box.bottom += s;
            return box
        }
    },
    ...
    getElementsByTagNameNS: function(node, ns, tgn) {
        node = node || document;
        var res = [];
        if (node.getElementsByTagNameNS) return node.getElementsByTagName(ns + ":" + tgn);
        else if (node.getElementsByTagName) {
            var n = document.namespaces;
            if (n.length > 0) {
                var l = node.getElementsByTagName(tgn);
                for (var i = 0,
                len = l.length; i < len; ++i) if (l[i].scopeName == ns) res.push(l[i])
            }
        }
        return res
    },
    getElementByTagNameBubble: function(elem, tn) {
        ...
        elem = MUSIC.dom.searchChain(elem = MUSIC.dom.get(elem), "parentNode",
        function(el) {
    ...
    insertAdjacent: function(elem, where, html, isText) {
        ...
        if (MUSIC.lang.isElement(elem) && (pos[where] && (MUSIC.lang.isString(html) || MUSIC.lang.isElement(html)))) {
            if (elem.insertAdjacentHTML) elem["insertAdjacent" + (typeof html == "object" ? "Element": isText ? "Text": "HTML")](pos[where], html);
            else {
...
MUSIC.util = {
    ...
    URI: function(s) {
        if (! (MUSIC.object.getType(s) == "string")) return null;
        ...
        if (MUSIC.object.getType(depart) == "array" && (depart.length > 1 && /^[a-zA-Z]+$/.test(depart[0]))) {
            ...
            if (MUSIC.object.getType(h) == "array") {
                ...
                if (this.search.length > 0 && this.hash.length > 0) if (ha < se) this.search = "";
                else this.search = depart[1].substring(se, ha);
                return this
            } else return null
        } else return null
    },
    ...
    buildArgs: function(args) {
        ...
        for (var key in args) {
            var value = args[key];
            if (typeof value == "string") buf.push(key + "=" + value)
        }
        return buf.join("&")
    },
    ...
    formatTime: function(iTime) {
        var sDate = "";
        try {
            iTime = parseInt(iTime) * 1E3;
            var oDate = new Date(iTime);
            var oNow = new Date;
            var iDiff = Math.floor(oNow.valueOf() / 864E5) - Math.floor(iTime / 864E5);
            switch (iDiff) {
            case 0:
                sDate = "\u4eca\u5929";
                break;
...
MUSIC.css = {
    ...
    getRulesBySheet: function(sheetId) {
        ...
        head,
        base;
        if (ss && !(rs = ss.cssRules || ss.rules)) if (head = document.getElementsByTagName("head")[0]) if (base = head.getElementsByTagName("base")[0]) {
    ...
    getRuleBySelector: function(sheetId, selector) {
        ...
        if (!_ss.cacheSelector) _ss.cacheSelector = {};
        if (_ss) {
            var _rs = _ss.cssRules || _ss.rules;
            var re = new RegExp("^" + selector + "$", "i");
            var _cs = _ss.cacheSelector[selector];
            if (_cs && re.test(_rs[_cs].selectorText)) return _rs[_cs];
            else {
                for (var i = 0; i < _rs.length; i++) if (re.test(_rs[i].selectorText)) {
    ...
    insertCSSLink: function(url, opts, callback) {
        ...
        if (ua.ie || ua.opera) {
            MUSIC.event.addEvent(cssLink, ua.ie && ua.ie < 9 ? "readystatechange": "load",
            function() {
                if (!cssLink || ua.ie && (ua.ie < 9 && !(cssLink.readyState == "loaded" || cssLink.readyState == "complete"))) return;
    ...
    insertStyleSheet: function(sheetId, rules) {
        ...
        if (rules) if (node.styleSheet) node.styleSheet.cssText = rules;
    ...
    swapClassName: function(elems, a, b, _isRep) {
        if (elems && typeof elems == "object") {
            ...
            for (var elem, i = 0,
            l = elems.length; i < l; ++i) if ((elem = elems[i]) && elem.nodeType == 1) if (MUSIC.css.hasClassName(elem, a)) MUSIC.css.updateClassName(elem, a, b);
            else if (!_isRep && MUSIC.css.hasClassName(elem, b)) MUSIC.css.updateClassName(elem, b, a)
        }
    },
    toggleClassName: function(elem, name) {
        if (!elem || elem.nodeType != 1) return;
        if (MUSIC.css.hasClassName(elem, name)) MUSIC.css.updateClassName(elem, name);
        else MUSIC.css.updateClassName(elem, null, name)
    }
};
MUSIC.debug = {
    errorLogs: [],
    ...
    showLog: function() {
        ...
        if ( !! o) o.innerHTML = MUSIC.string.nl2br(MUSIC.string.escHTML(this.errorLogs.join("\n")))
    },
...
MUSIC.runTime = function() {
    ...
    function log(msg, type) {
        var info;
        if (isDebugMode()) info = msg + "\n=STACK=\n" + stack();
        else if (type == "error") info = msg;
        else if (type == "warn");
    ...
    function stack(e, a) {
        function genTrace(ee, aa) {
            if (ee.stack) return ee.stack;
                ...
                var r = [];
                for (var i = 0,
                len = eas.length; i < len; i++) r.push(typeof eas[i] == "undefined" ? "<u>": eas[i] === null ? "<n>": eas[i]);
        ...
        var res;
        if (e instanceof Error && (typeof arguments == "object" && !!arguments.callee)) res = genTrace(e, a);
        else try { ({}).sds()
        } catch(err) {
...
MUSIC.enviroment = function() {
    ...
    function envSet(kname, value) {
        if (typeof value == "undefined") if (typeof kname == "undefined") return false;
        else {
            if (! (_p[kname] === undefined)) return false
        } else {
            _p[kname] = value;
            return true
        }
...
MUSIC.pageEvents = function() {
    function _ihp() {
        ...
        if (s && s.DEBUG) {
            ...
            if (!isNaN(n)) MUSIC.config.debugLevel = n
        }
    }
    function _bootStrap() {
        if (document.addEventListener) if (MUSIC.userAgent.safari < 4) var interval = setInterval(function() {
        ...
        50);
        else document.addEventListener("DOMContentLoaded", _onloadHook, true);
        else {
            var src = "javascript:void(0)";
            if (window.location.protocol == "https:") src = "//:";
            document.write('<script onreadystatechange="if(this.readyState==\'complete\'){this.parentNode.removeChild(this);MUSIC.pageEvents._onloadHook();}" defer="defer" src="' + src + '">\x3c/script>')
        }
        window.onload = MUSIC.lang.chain(window.onload,
        function() {
            _onloadHook();
        ...
        window.onunload = MUSIC.lang.chain(window.onunload,
        function() {
    ...
    function _runHooks(hooks) {
        ...
        do {
            ...
            if (!h) break;
            for (var ii = 0; ii < h.length; ii++) if (isbeforeunload) warn = warn || h[ii]();
            else h[ii]();
            if (isbeforeunload) break
        } while ( hc [ hooks ]);
        if (isbeforeunload) if (warn) return warn;
...
MUSIC.object.extend(MUSIC.console, {
    ...
    _init: function() {
        ...
        if (MUSIC.dragdrop) MUSIC.dragdrop.registerDragdropHandler(MUSIC.dom.get("log_head"), this._main);
...
MUSIC.CookieSet = function() {
    ...
    return {
        ...
        get: function() {
            ...
            var data = {};
            MUSIC.object.each(args,
            function(arg, idx) {
                var value = parts[idx];
                data[arg] = value
            });
            if (typeof this.check_func == "function") if (!this.check_func(data)) {
                this.clear();
                return null
            }
            return data
        ...
        set: function(data) {
            var key = this.key();
            if (!key) return false;
            if (typeof this.check_func == "function") if (!this.check_func(data)) return false;
            var parts = [];
...
var delCookie = MUSIC.cookie.del; (function(qdc) {
    var dataPool = {};
...
MUSIC.XHR.xsend = function(o, uri) {
    ...
    if (o._sender === null || o._sender === void 0) {
        ...
        sender.callback = MUSIC.event.bind(o,
        function(data) {
            o.onSuccess(data);
            clear(o)
        });
        sender.errorCallback = MUSIC.event.bind(o,
        function(num) {
    ...
    o._sender.src = uri.protocol + "://" + uri.host + (o.proxyPath ? o.proxyPath: o.charset == "gb2312" ? "/xhr_proxy_gbk.html": "/xhr_proxy_utf8.html");
    return true
};
...
MUSIC.XHR.prototype.send = function() {
    ...
    if (!this._sender) {
        ...
        else if (window.ActiveXObject) try { ! (sender = new ActiveXObject("Msxml2.XMLHTTP")) && (sender = new ActiveXObject("Microsoft.XMLHTTP"))
        } catch(ign) {}
    ...
    this._sender.onreadystatechange = MUSIC.event.bind(this,
    function() {
        try {
...
MUSIC.media = {
    ...
    adjustImageSize: function(objImage, url, maxwidth, maxheight, callback) {
        ...
        image.onload = function(mainImg, tempImg, mw, mh) {
            return function() {
                ...
                if (typeof callback == "function") callback(mainImg, mw, mh, tempImg, _w, _h);
                try {
    ...
    getFlashHtml: function(flashArguments, requiredVersion, flashPlayerCID) {
        ...
        if (ua.ie) return "<object classid='clsid:" + flashPlayerCID + "' " + _attrs + ">" + _params + "</object>";
...
MUSIC.media.SWFVersion = function() {
    ...
    else if (arguments.length == 1) if (typeof arguments[0] == "object") a = arguments[0];
...
MUSIC.FormSender.prototype.send = function() {
    ...
    if (this._sender === null || this._sender === void 0) {
        ...
        document.body.appendChild(sender);
        sender.callback = MUSIC.event.bind(this,
        function(o) {
            clearTimeout(timer);
            this.onSuccess(o);
        ...
        sender.errorCallback = MUSIC.event.bind(this,
        function(o) {
            clearTimeout(timer);
            this.onError(o);
            this.resultArgs = arguments;
        ...
        if (typeof sender.onreadystatechange != "undefined") sender.onreadystatechange = MUSIC.event.bind(this,
        function() {
        ...
        else var timer = setTimeout(MUSIC.event.bind(this,
        function() {
            try {
...
MUSIC.JsLoader.prototype._load = function(src, doc, opts) {
    ...
    function() {
        if (!_js || _ie && (_ie < 10 && (typeof docMode == "undefined" || docMode < 10 ? _js.readyState != "loaded": _js.readyState != "complete"))) return;
        _ie && idp.push(_js.id);
        _js._inUse = false;
        o.onload(); ! _ie && _rm(_js);
        _js = _ae = o = null
    });
    if (!_ie) _ae(_js, "error",
    function() {
        _ie && idp.push(_js.id);
        _js._inUse = false;
...
MUSIC.JSONGetter = function(actionURL, cname, data, charset, junctionMode) {
    ...
    if (typeof actionURL == "string") if (actionURL.indexOf("g_tk=") < 0) {
        var jsRE = /\.(js|json)$/i;
        var sw, pn;
        ...
        j = _elems.length; i < j; i++) {
            ...
            if (_isVisible(range, _elems[i])) {
                ...
                if ( !! _src) {
                    ...
                    if (_w > 0 && _h > 0) MUSIC.media.adjustImageSize(_elem, _src, _w, _h);
                    else _elem.src = _src;
                    delete _elems[i];
                    _count--;
                    continue
...
MUSIC.widget.trackServ = {
    _arrSongAttr: ["mid", "msong", "msingerid", "msinger", "malbumid", "malbum", "msize", "minterval", "mstream", "mdownload", "msingertype", "size320", "size128", "mrate", "gososo", "sizeape", "sizeflac", "sizeogg", "sizetac", "downstatus", "mmid", "msingermid", "malbummid", "mstatus", "mnote"],
    _arrSongAttrSoso: ["mid", "msong", "malbum", "msinger", "msize", "mformat", "mrate", "minterval", "msongurl", "msingerid", "malbumid", "mdownload", "mshowurl"],
    ...
    formatMusic: function() {
        ...
        var songdata = "";
        if (MUSIC.lang.isString(arguments[0])) songdata = arguments[0];
        else if (MUSIC.lang.isNode(arguments[0])) songdata = this.getSongData(arguments[0]);
        ...
        for (i = 0; i < _length; i++) _obj[this._arrSongAttr[i]] = arrMusic[i];
        for (i = _length; i < length; i++) _obj[this._arrSongAttr[i]] = "";
        _obj.mtype = "qqmusic";
        return _obj
    },
    formatMusic_Soso: function(songdata) {
        ...
        _length = Math.min(length, arrMusic.length);
        var _obj = {};
        for (i = 0; i < _length; i++) _obj[this._arrSongAttrSoso[i]] = arrMusic[i];
        for (i = _length; i < length; i++) _obj[this._arrSongAttrSoso[i]] = "";
    ...
    buildMusicData: function(music_info, count) {
        ...
        if (music_info.mtype == "net") return this.buildMusicDataSoso(music_info);
        var _list = [];
        for (var i = 0; i < count; i++) _list.push(music_info[this._arrSongAttr[i]]);
        return _list.join("|")
    },
    buildMusicDataSoso: function(music_info, count) {
        count = count || this._arrSongAttrSoso.length;
        var _list = [];
        for (var i = 0; i < count; i++) _list.push(music_info[this._arrSongAttrSoso[i]]);
        return _list.join("@@")
    },
    showSingleSong: function(obj, subtype) {
        ...
        if (MUSIC.userAgent.ie == 6) setTimeout(function() {
            window.location.href = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_yqq_song_detail_info.fcg?songmid=" + _music.mmid
        },
        0);
        else location.href = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_yqq_song_detail_info.fcg?songmid=" + _music.mmid
    },
    play: function(obj, subtype) {
        ...
        if (parseInt(_music.mstatus) == 0 && (parseInt(_music.mnote) >= 4E3 && parseInt(_music.mnote) <= 4999)) {
            g_popup.show(1, "\u6b4c\u66f2\u64ad\u653e\u5931\u8d25\uff01", "\u62b1\u6b49\uff0c\u56e0\u7248\u6743\u539f\u56e0\uff0c\u6682\u4e0d\u63d0\u4f9b\u8be5\u6b4c\u66f2\u64ad\u653e\u670d\u52a1\uff01", "", 3E3, 360);
            return
        }
        if (_music.mtype == "qqmusic" && (parseInt(_music.msize) <= 0 || parseInt(_music.minterval) <= 0)) {
            g_popup.show(1, "\u6b4c\u66f2\u64ad\u653e\u5931\u8d25\uff01", "\u5f88\u62b1\u6b49\uff0cQQ\u97f3\u4e50\u6682\u4e0d\u63d0\u4f9b\u6b64\u6b4c\u66f2\u670d\u52a1\u3002", "", 3E3, 360);
            return
        }
    ...
    shareMusic: function(obj, subtype) {
        ...
        pageHotClick();
        var _music = null;
        if (typeof obj.msong != "undefined") _music = obj;
        else {
            try {
                _music = this.formatMusic(obj.parentNode.parentNode)
            } catch(e) {
    ...
    like: function(obj, subtype) {
        ...
        if (obj.className == "btn_like") {
            top.g_mymusicAlbumMod.Song.like(_music.mmid, 3,
            function() {
            ...
            top.g_mymusicAlbumMod.Song.unlike(_music.mmid, 3,
            function() {
        ...
        function(_elem) {
            ...
            function(e) {
                ...
                e.onmouseout = function() {
                    if (!MUSIC.css.hasClassName(e, "class_fav")) MUSIC.css.removeClassName(e, classHover)
                };
                try {
                    e.draggable = "true";
                    MUSIC.event.addEvent(e, "dragstart",
                    function(evt) {
                        try {
                    ...
                    MUSIC.event.addEvent(e, "dragend",
                    function(evt) {
                        try {
                            top["drag_songinfo"] = "";
            ...
            g_popup.init({
                ...
                contents: function(callback) {
                    ...
                    top.g_mymusicAlbumMod.getList(uin,
                    function(list) {
                        var arr = [];
                        for (var i = 0,
                ...
                showCall: function(obj) {
                    var obj = MUSIC.dom.searchChain(obj, null,
                    function(elem) {
    ...
    getSongDataList: function(elem) {
        ...
        try {
            for (var j = 0; j < ulList.length; j++) {
                ...
                if (parseInt(_music.mstatus) == 0 && (parseInt(_music.mnote) >= 4E3 && parseInt(_music.mnote) <= 4999)) flag = true;
                else if (_music.mid != "") _list.push(_music)
            }
            if (list.length == 0 && flag) g_popup.show(1, "\u6b4c\u66f2\u64ad\u653e\u5931\u8d25\uff01", "\u62b1\u6b49\uff0c\u56e0\u7248\u6743\u539f\u56e0\uff0c\u6682\u4e0d\u63d0\u4f9b\u8be5\u6b4c\u66f2\u64ad\u653e\u670d\u52a1\uff01", "", 3E3, 360)
        } catch(ex) {
            if (_list.length <= 0) {
                ...
                for (var j = 0; j < ulList.length; j++) {
                    ...
                    if (input.length > 0 && input[0].checked == false) continue;
                    var _music = this.formatMusic(ulList[j]);
                    if (_music.mid != "") _list.push(_music)
                }
            }
        }
        return _list
    ...
    select: function(e, bStatus) {
        ...
        try {
            ...
            for (var j = 0,
            length = lstMusic.length; j < length; j++) if (!lstMusic[j].disabled && (lstMusic[j].type && lstMusic[j].type == "checkbox")) lstMusic[j].checked = bStatus
        } catch(e) {}
    },
    ...
    getSongButtonHtml: function(objMusic, stattype) {
        ...
        if ((parseInt(objMusic.msize) <= 0 || parseInt(objMusic.minterval) <= 0) && objMusic.mtype != "net" || objMusic.mstatus == 0 && (objMusic.mnote >= 4E3 && objMusic.mnote <= 4999)) {
        ...
        if (objMusic.mstatus == 0 && (objMusic.mnote >= 8E3 && objMusic.mnote <= 8999)) obj.btn_soso = '<i class="icon_sosomusic" title="\u8be5\u6b4c\u66f2\u6765\u81ea\u7b2c\u4e09\u65b9\u7f51\u7ad9"></i>';
        else obj.btn_soso = "";
        if (_gososo != 0 || (_down != 1 || objMusic.mstatus == 0 && (objMusic.mnote >= 4E3 && objMusic.mnote <= 4999))) {
            obj.btn_down = "";
    ...
    getAlbumInfoById: function(albummid, callback) {
        ...
        url = "http://data.music.qq.com/song/list.json?order=seq&albummid=" + albummid,
    ...
    playAlbum: function(albummid, subtype) {
        ...
        pageHotClick();
        var _ = this;
        this.getAlbumInfoById(albummid,
        function(data) {
            var list = [],
            flag = false;
            MUSIC.object.each(data.data.list,
            function(info) {
                var _music = _.formatMusic(info.musicData.unescapeHTML());
                if (parseInt(_music.mstatus) == 0 && (parseInt(_music.mnote) >= 4E3 && parseInt(_music.mnote) <= 4999)) flag = true;
                else list.push(_music)
            });
            if (list.length == 0 && flag) g_popup.show(1, "\u6b4c\u66f2\u64ad\u653e\u5931\u8d25\uff01", "\u62b1\u6b49\uff0c\u56e0\u7248\u6743\u539f\u56e0\uff0c\u6682\u4e0d\u63d0\u4f9b\u8be5\u6b4c\u66f2\u64ad\u653e\u670d\u52a1\uff01", "", 3E3, 360);
            else top.g_topPlayer.add(list, true)
        });
    ...
    getSongsCount: function(container, tagName, flag, index) {
        ...
        var url = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_getsonglistenstatistic.fcg?utf8=1";
        ...
        j.onSuccess = function(data) {
            if (!data.code) {
                var max_listen = 0;
                for (var i = 0; i < data.songlist.length; i++) if (data.songlist[i].count > max_listen) max_listen = data.songlist[i].count;
                for (var i = 0; i < data.songlist.length; i++) {
                    ...
                    if (_music.mstatus == 0 && (_music.mnote >= 4E3 && _music.mnote <= 4999)) {
                        perCount = 0;
                        Count = "0\u4eba\u542c\u8fc7"
                    }
                    if (tagName == "span" && !flag) if (_music.mstatus == 0 && (_music.mnote >= 4E3 && _music.mnote <= 4999)) _items[i + index].innerHTML = "0";
    ...
    jumpToMidPage: function(opt) {
        ...
        opt.play = opt.play || 0;
        var url = "http://s.plcloud.music.qq.com/fcgi-bin/fcg_id_jump_mid.fcg?g_tk=" + getACSRFToken(),
        ...
        if (typeof opt.mid === "undefined" || (opt.mid === "" || opt.mid === "0")) {
            ...
            if (p === "singerid") url = "//y.qq.com/y/static/singer" + g_mid.init({
                mid_flag: 0
            }).getSingerFile({
                id: opt.singerid,
                mid: opt.mid
            });
            else if (p === "albumid") url = "//y.qq.com/y/static/album" + g_mid.init({
                mid_flag: 0
            }).getAlbumFile({
                id: opt.albumid,
                mid: opt.mid
...
MUSIC.widget.pager = {
    ...
    get: function(params) {
        ...
        var obj = this;
        var ret = null; (function pageLoad() {
            try {
    ...
    pageNo: function(total, currentPage, perPage, showIndex, ns) {
        ...
        if (total / perPage > parseInt(total / perPage)) pages = parseInt(total / perPage) + 1;
        ...
        pageIndex.push("");
        if (pages <= 2 * showIndex) for (var i = 1; i <= pages; i++) if (i == currentPage) pageIndex.push('<strong class="current">' + i + "</strong>");
            ...
            if (endIndex - startIndex < showIndex) startIndex = endIndex - showIndex < 1 ? 1 : endIndex - showIndex;
...
MUSIC.widget.user = {
    ...
    openLoginPage: function(target) {
        ...
        var loginUrl = "";
        if (isIphoneOs) loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?style=8&appid=1006102&s_url=" + encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
        else if (isAndroid) loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?style=9&appid=1006102&s_url=" + encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
        else loginUrl = "http://y.qq.com/y/static/login.html#url=" + encodeURIComponent(_url);
        if (isIphoneOs || isAndroid) if ( !! target && target == "parent") top.window.location.href = loginUrl;
    ...
    openLogin: function(url, target, logincallback, closecallback) {
        ...
        if (isIphoneOs || isAndroid) {
            var loginUrl = "",
            _url = (top.window.frames["contentFrame"] || window).location.href;
            if (isIphoneOs) loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?style=8&appid=1006102&s_url=" + encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
            else loginUrl = "http://ui.ptlogin2.qq.com/cgi-bin/login?style=9&appid=1006102&s_url=" + encodeURIComponent(_url) + "&low_login_enable=0&hln_css=&hln_title=&hln_acc=&hln_pwd=&hln_u_tips=&hln_p_tips=&hln_autologin=&hln_login=&hln_otheracc=&hln_qloginacc=&hln_reg=&hln_vctitle=&hln_verifycode=&hln_vclogin=&hln_feedback=";
    ...
    getVipInfo: function(callBack, errCallBack) {
        ...
        var url = "http://portalcgi.music.qq.com/fcgi-bin/music_mini_portal/fcg_getuser_infoEx.fcg?outCharset=utf-8&rnd=" + (new Date).valueOf(),
        ...
        j.onError = function() {
            ...
            else g_popup.show(1, "\u8bfb\u53d6\u7528\u6237\u8eab\u4efd\u4fe1\u606f\u5931\u8d25\uff01", "\u5f53\u524d\u7f51\u7edc\u7e41\u5fd9\uff0c\u8bf7\u60a8\u7a0d\u540e\u518d\u8bd5\u3002", 3E3, 300)
        };
    ...
    getQzoneUserImage: function(uin, size) {
        ...
        uin = parseInt(uin, 10);
        if (uin < 10001) return "http://imgcache.qq.com/minimusic_v2/theme1/img/initial_face_big_pic.gif";
    ...
    getQQUserImage: function(uin) {
        ...
        if (uin < 10001) return "http://imgcache.qq.com/minimusic_v2/theme1/img/initial_face_big_pic.gif?max_age=2592000";
    ...
    getBatchedQQUserImage: function(uinlist, callback) {
        if (!uinlist) { !! callback && callback(null);
            return
        }
        var uin = g_user.getUin(),
        ...
        jg.onSuccess = function(json) { !! callback && callback(json)
        };
        jg.onTimeout = jg.onError = function() {
            ...
            for (var i = 0,
            uin; uin = uinlist.split(",")[i++];) json.data.urllist.push("http://imgcache.qq.com/minimusic_v2/theme1/img/initial_face_big_pic.gif"); !! callback && callback(json)
        };
        jg.send(jsonpCallback)
    }
};
var UserInfoCookie = MUSIC.CookieSet("vip", "nickname", "score", "place", "payway", "start", "end", "yearFlag", "yearstart", "yearend", "nowtime", "CurrentLevel", "UpgradeDayLeft", "NextLevelPoint", "UpgradePercentLeft", "EverydayPoint", "PrepaidByBankcard");
...
function pageHotClick() {
    ...
    if (getStatSource()) if (typeof pgvWatchClick != "undefined") pgvWatchClick({
        statIframe: true
    })
}
MUSIC.widget.main = {
    init: function() {
        if (top == window && (/^http:\/\/(y|music)\.qq\.com\/y\/static\/(?!mv\/mv_play\.html)/.test(window.location.href) || /^http:\/\/s\.plcloud\.music\.qq\.com\/fcgi-bin\/fcg_yqq_song_detail_info\.fcg/.test(window.location.href))) {
            ...
            else window.location.href = "http://y.qq.com/#" + simpleUrl;
            return
        }
        try {
            try {
            ...
            if (top != window && top.window.location.href.indexOf("y.qq.com") > -1) {
                g_hashHistory.init(function(hash_obj) {
                    ...
                    if (simpleUrlObj["type"] != "" && simpleUrlObj["type"] != hash_obj["type"]) history.back();
    ...
    reloadImg: function(container) {
        try {
            ...
            length = arrImg.length; i < length; i++) {
                if (arrImg[i].readyState == "uninitialized") if (arrImg[i].src.search(/singer|album|topic|music_topic/) > 0) arrImg[i].src = "http://imgcache.qq.com/mediastyle/y/img/cover_mine_130.jpg"
            } else for (var i = 0,
            length = arrImg.length; i < length; i++) if (arrImg[i].src.search(/singer|album|topic|music_topic/) > 0) {
    ...
    watchPage: function() {
        ...
        var _id = 0;
        var _page = [/\/y\/static\/index\./, /\/singer\/index\//, /\/singer\/\d+\//, /\/singer\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)/, /\/album\/(album_list|shoufa)\//, /\/album\/\d+\//, /\/album\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)/, /top_index\.html/, /\/toplist\/index\//, /\/toplist\/albumtop\//, /top_mv\.html/, /\/mv\/index\./, /\/mv\/lib\//, /mymusic_index\.html/, /mymusic_albumlist\.html/, /mymusic_mvlist\.html/, /mymusic_friend\.html/, /fcg_yqq_song_detail_info\.fcg/, /\/mv\/mv_play\.html/, /soso\.music\.qq\.com/, /(v5|y\.qq\.com)\/(index|\?|\#|$)/];
        for (var i = 0,
        _length = _page.length; i < _length; i++) if (_url.search(_page[i]) > 0) {
            _id = i + 1;
            break
        }
    ...
    IP_limit: function() {
        ...
        function loadIPcgi(callback) {
            ...
            var url = "http://s.plcloud.music.qq.com/fcgi-bin/p.fcg";
...
MUSIC.widget.tips = {
    ...
    showMask: function() {
        ...
        if (!_elem) _elem = MUSIC.dom.createElementIn("div", null, false, {
            id: "divMaskPage",
...
MUSIC.widget.tips.popup = function() {
    ...
    function show2(options) {
        ...
        _insertCss(function() {
            ...
            if (!_e) _e = $D.createElementIn("div", null, false, {
                id: "divPopup",
                style: "position:absolute;z-index:10001;width:" + _opt.width + "px;",
        ...
        function(_e) {
            ...
            case "click":
                ...
                function() {
                    ...
                    else if (_opt.isToggle) if (_elemPopup.style.display == "none") {
                        _curTarget = _e;
                        _show()
                    } else _hide();
                    else {
                ...
                if (top != window && window.location.href.indexOf("fm.qq.com") < 0) $E.on(top.document, "click",
                function() {
                    if (_elemPopup && _elemPopup.style.display != "none") _hide()
                });
                break;
...
MUSIC.widget.tips.dialog = function() {
    ...
    function show(options) {
        ...
        $.object.extend(_opt, options || {});
        if (_opt.mode == "iframe") if (window.location.href.indexOf("y.qq.com") < 0) _opt.isLoadCss = false;
        _insertCss(function() {
...
MUSIC.widget.watch = {
    ...
    setTime: function() {
        ...
        _end = new Date - 0;
        if (_begin == 0) _begin = g_watchCssBegin;
    ...
    send: function(_id, _rnd) {
        ...
        if ((new Date).valueOf() % this._rnd == 0) {
            ...
            url += "&flag3=" + _id;
            url += "&flag4=" + this._rnd;
            for (var i = 0; i < this.timers.length; i++) url += "&" + (i + 1) + "=" + this.timers[i]; (new Image).src = url
        }
    },
    commit: function(id) {
...
MUSIC.widget.statistics = {
    ...
    stat: function(optcode, dim1, dim2, song_id, source, buff_time, rule_id) {
        ...
        function _stat() {
            ...
            for (var i = 0,
            len = item.length; i < len; i++) arg_list.push("a0-" + i + "=" + item[i]);
            if (optcode > 0) {
...
function stat_miniblog(singerid) {} (function(q) {
    ...
    function valueStat(domain, cgi, type, code, time, rate, uin, exts) {
        ...
        if (domain != "" || i > 0) try {
            var url = commurl + "?" + param.join("&"); (new Image).src = url
        } catch(e) {}
    }
    function _r() {
        if (collector.length) valueStat("", "", "", "", "", "", uin, collector);
            ...
            q[n].pluginsPool.onRequestComplete.push(function(th) {
                ...
                if (d && (d = d[0])) {
                    ...
                    if (typeof d.code == "undefined") return;
                        ...
                        if (th instanceof q.JSONGetter) if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 1E3]);
                        else isreportG && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 1E3]);
                        if (th instanceof q.FormSender) if (th.reportRate)(th.reportRate == 1 || Math.random() < 1 / th.reportRate) && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 10]);
                        else isreportP && collector.push([domain, url, 1, d.subcode || 1, +th.endTime - th.startTime, th.reportRate || 10])
                    }
                }
            })
        }
...
MUSIC.widget.Timer.prototype = {
    ...
    onTimerEvent: function() {
        if (!this.isExecuting) try {
            ...
            this.times++;
            if (typeof this.callback == "function") this.callback(this.times);
...
MUSIC.widget.Storage.create = function(cb, opt) {
    ...
    _backend.localStorage = {
        ...
        for (var i = 0,
        len = _clientStore.length; i < len; i++) if (_backend[_clientStore[i]].isSupport) { (_cs["instance"] = _backend[_clientStore[i]]).init();
            return
        }
        cb(false)
    })()
...
MUSIC.widget.hashHistory = function() {
    ...
    function param2obj(str) {
        ...
        var arr = str.split("&");
        for (var i = 0,
        len = arr.length; i < len; i++) if (arr[i].indexOf("=") > -1) {
            var kv = arr[i].split("=");
            obj[kv[0]] = kv[1]
        }
        return obj
    ...
    function getSimpleUrl(url) {
        ...
        else if (/\/y\/static\/singer\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)\.html/.test(url)) hash = "type=singer&mid=" + RegExp.$3;
        else if (/\/y\/static\/singer\/index\/([^#?$.]*)/.test(url)) hash = "type=singer&p=" + encodeURIComponent(RegExp.$1);
        else if (/\/y\/static\/album\/(\d+)\/album_(\d+)_1\.html/.test(url)) hash = "type=album&id=" + RegExp.$2;
        else if (/\/y\/static\/album\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)\.html\?play=([^#?$.]*)/.test(url) || /\/y\/static\/album\/([^#?$.]*)\/([^#?$.]*)\/([^#?$.]*)\.html/.test(url)) hash = "type=album&mid=" + RegExp.$3 + (RegExp.$4 ? "&play=" + RegExp.$4: "");
        ...
        else if (/\/y\/static\/toplist\/index\/([^#?$.]*)/.test(url)) hash = "type=toplist&p=" + encodeURIComponent(RegExp.$1);
        else if (/\/y\/static\/toplist\/albumtop\/([^#?$.]*)/.test(url)) hash = "type=albumtop&p=" + encodeURIComponent(RegExp.$1);
        else if (/\/y\/static\/index\.html/.test(url)) hash = "type=index";
        else if (/\/y\/static\/mymusic\/([^#$]*)/.test(url)) {
            if (RegExp.$1.indexOf("mymusic_index") || (RegExp.$1.indexOf("mymusic_albumlist") || (RegExp.$1.indexOf("mymusic_mvlist") || (RegExp.$1.indexOf("mymusic_follow_listen") || RegExp.$1.indexOf("mymusic_follow_listen"))))) hash = "type=mymusic&p=" + encodeURIComponent(RegExp.$1)
        } else if (/\/y\/static\/album\/song_detail.html#songid=(\d+)/.test(url) || /s\.plcloud\.music\.qq\.com\/fcgi-bin\/fcg_yqq_song_detail_info.fcg\?songid=(\d+)/.test(url)) hash = "type=song&id=" + RegExp.$1;
        else if (/\/y\/static\/album\/song_detail.html#songmid=([^#?$.]*)/.test(url) || /s\.plcloud\.music\.qq\.com\/fcgi-bin\/fcg_yqq_song_detail_info.fcg\?songmid=([^#?$.]*)/.test(url)) hash = "type=song&mid=" + RegExp.$1;
        else if (/\/soso.music.qq.com\/fcgi-bin\/multiple_music_search.fcg([^#$]*)/.test(url)) hash = "type=soso&p=" + encodeURIComponent(RegExp.$1);
    ...
    function locationHash(hash_obj) {
        ...
        if (simpleUrl == "") {
            ...
            for (var name in obj) if (obj[name] != topUrlObj[name]) {
                isIgnore = true;
                break
            }
            for (var name in topUrlObj) if (obj[name] != topUrlObj[name]) {
                isIgnore = true;
                break
            }
        }
...
MUSIC.widget.mid = function() {
    ...
    function getSingerPic(op) {
        ...
        } else if (_opt.mid_flag == 0 && (typeof op.id !== "undefined" && op.id != 0)) if (op.type == 150 || (op.type == 90 || op.type == 68)) picRtn = picNomal.jstpl_format({
            singerId: op.id,
...
/*  |xGv00|e13767f89d4721870435ae3377729b76 */
